...
function db_dump () {
    local db_name=$1
    local file_name=$2
    ${MDUMP} --user=${DBUSER} --password=${DBPASSWORD} --host=${HOST} ${OPTS} --databases ${db_name} 2>${TMP} > ${file_name}
    last_eroror=$?
    if [ $last_eroror -eq 0 ]
    then
        ls ${file_name};
...
exit;
    ...
    # clean old backups
    [ -n "$BKCOUNT" ] && {
ls -t "$BACKUPDIR" | tail -n +$((BKCOUNT+1)) | xargs printf -- "$BACKUPDIR\/%s\n" | xargs rm 2>/dev/null
    }
}

function usage() {
    echo "Usage:  `basename $0` -m dump -c count -u user -p password -d database -t tables";
    echo -e "\t`basename $0` -m dumpall -c count -u user -p password";
}
